...
namespace Hanger51.EditorTools
{
    public static class Hanger51UrpLightSubsetDiagnostics
    {
        ...
        private const string InputSystemUiType = "UnityEngine.InputSystem.UI.InputSystemUIInputModule";
        private const string UrpCameraType = "UnityEngine.Rendering.Universal.UniversalAdditionalCameraData";
        private const string UrpLightType = "UnityEngine.Rendering.Universal.UniversalAdditionalLightData";
        ...
        public static void ReportUrpLightDataPaths()
        {
            ...
            for (int rootIndex = 0; rootIndex < roots.Length; rootIndex++)
            {
                ...
                for (int index = 0; index < behaviours.Length; index++)
                {
                    ...
                    if ((behaviour.GetType().FullName ?? behaviour.GetType().Name) != UrpLightType) continue;
            ...
            builder.Append("URP light-data report for '").Append(scene.path).Append("': found ")
                .Append(paths.Count).Append(" component(s).");

            for (int index = 0; index < paths.Count; index++)
            {
                builder.Append("\n  [").Append(index).Append("] ").Append(paths[index]);
            }
        ...
        private static void BuildVariant(int lightLimit)
        {
            ...
            try
            {
                ...
                for (int rootIndex = 0; rootIndex < sourceRoots.Length; rootIndex++)
                {
                    ...
                    allBehaviours.AddRange(clone.GetComponentsInChildren<MonoBehaviour>(true));
                }
                ...
                lightBehaviours.Sort((a, b) => string.Compare(GetPath(a.transform), GetPath(b.transform), StringComparison.Ordinal));

                int totalLights = lightBehaviours.Count;
                int keepLightCount = lightLimit == int.MaxValue ? totalLights : Mathf.Min(lightLimit, totalLights);
                ...
                Debug.Log(
                    ...
                    + "Kept Hanger51=" + keptHanger
                    + ", InputUI=" + keptInput
                    + ", URPCamera=" + keptCamera
                    + ", URPLights=" + keepLightCount + "/" + totalLights
                    + ", removed other MonoBehaviours=" + removedOther
                ...
                Debug.Log(
                    "URP light-subset diagnostic '" + variantFolder + "' build passed. Size="
                    + report.summary.totalSize + " bytes; duration=" + report.summary.totalTime + ". Launching now.");
