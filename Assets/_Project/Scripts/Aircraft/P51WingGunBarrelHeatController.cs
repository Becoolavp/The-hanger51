...
namespace Hanger51.Aircraft
{
    ...
    public sealed class P51WingGunBarrelHeatController : MonoBehaviour
    {
        ...
        [SerializeField] private Renderer[] barrelRenderers = new Renderer[GunCount];
        ...
        private readonly Material[] runtimeMaterials = new Material[GunCount];
        ...
        public void Configure(P51WingArmamentSystem configuredSystem, Renderer[] configuredBarrelRenderers)
        {
            ...
            barrelRenderers = new Renderer[GunCount];
            if (configuredBarrelRenderers != null)
            {
                int count = Mathf.Min(GunCount, configuredBarrelRenderers.Length);
        ...
        private void ApplyHeat(int stationIndex)
        {
            ...
            Material material = runtimeMaterials[stationIndex];
            if (material == null)
            {
                return;
            }
        ...
        {
            if (barrelRenderers == null || barrelRenderers.Length != GunCount)
            {
                Renderer[] resized = new Renderer[GunCount];
                if (barrelRenderers != null)
                {
                    int count = Mathf.Min(GunCount, barrelRenderers.Length);
            ...
            for (int stationIndex = 0; stationIndex < GunCount; stationIndex++)
            {
                Renderer renderer = barrelRenderers[stationIndex];
                if (renderer == null || runtimeMaterials[stationIndex] != null)
                {
                    continue;
                }
                ...
                {
                    name = $"P-51 Wing Gun {stationIndex + 1} Heated Barrel Material"
                };
                renderer.material = runtime;
                runtimeMaterials[stationIndex] = runtime;
            }
        }
